...
async def handle_request(reader, writer):
    try:
        ...
        save_path = request.get("save_path", str(Path.home() / "Downloads"))
        ...
        if req_type == "add_torrent":
            try:
                if source.startswith("magnet:"):
                    ...
                    resume_file = TORRENT_DATA_DIR / f"{info_hash}.fastresume"
                    if resume_file.exists():
                        resume_data = resume_file.read_bytes()
                        entry = lt.bdecode(resume_data)
                        params["resume_data"] = entry
            ...
            if handle:
                ...
                fastresume_file = TORRENT_DATA_DIR / f"{source}.fastresume"
                torrent_file = TORRENT_DATA_DIR / f"{source}.torrent"
                for file_path in (fastresume_file, torrent_file):
                    if file_path.exists():
                        file_path.unlink()
            ...
            if handle:
                ...
                progress_data = {
                    "name": handle.name(),
                    "status": "seeding" if status.is_seeding else ("paused" if status.paused else "downloading"),
                    "download_progress": round(status.progress * 100, 2),
                    "time_left": time_left,
